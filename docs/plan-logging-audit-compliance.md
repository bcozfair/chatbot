...
| สถานการณ์ | ไม่มี trigger | มี trigger | แถว audit ที่เขียน |
| --- | --- | --- | --- |
| แก้กฎทีละตัวจากหน้าจอ (เคสปกติ) | — | **+0.14 ms** | 1 |
| กดยกสายเดิมซ้ำด้วยค่าเดิม (5,274 แถว) | — | 426 ms | **0** (ไม่มีอะไรเปลี่ยน = ไม่เขียน) |
| `POST /api/admin/stock-rules` ยกทั้งสายที่ใหญ่ที่สุด<br>`Production 2(PM)` = 35,141 รายการ **ที่ค่าเปลี่ยนจริง** | 672 ms | **4,106 ms** | **35,141** |

⚠️ **ข้อค้นพบที่ต้องตัดสินใจ** — แถวสุดท้ายคือเคสแย่ที่สุดที่วัดได้จริง: การกดปุ่มเดียวช้าลง 6 เท่า
(ยังต่ำกว่า `statement_timeout` 15 วิของแอป จึงไม่พัง) และเขียน audit 35,141 แถวจากการกด 1 ครั้ง
ซึ่งจะกลบรายการแก้ไขจริงในหน้าจอจนอ่านไม่ออก
ทางเลือกที่มี: (ก) ปล่อยไว้ — ถูกต้องตามความหมาย แต่หน้าจอรก · (ข) ถอด `trg_audit` ออกจาก
`product_stock_rules` / `product_moq_rules` / `product_optional_links` ซึ่งเป็น "ข้อมูลตั้งค่ายกชุด"
ไม่ใช่ "การตั้งค่าเชิงนโยบาย" · (ค) เปลี่ยนเป็น statement-level trigger + transition table
สรุปเป็น 1 แถวต่อ 1 คำสั่ง — ตรงกลางที่ดีที่สุด แต่ต้องแก้ `audit_row()`
**ยังไม่ทำอะไรทั้งสิ้น รอเจ้าของระบบตัดสิน** — สามตารางที่เหลืออีก 8 ตัวไม่มีปัญหานี้ (แถวหลักหน่วยถึงหลักสิบ)
