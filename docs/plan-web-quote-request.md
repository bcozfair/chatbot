...
> **สถานะ: v5.1 — form-first + ตัวตนผู้เสนอราคาบน PDF · ✅ เฟส B เสร็จแล้ว · เฟส A ยกเลิก · ถัดไปคือเฟส B2 หรือ C**
...
> · **v5.1 เจ้าของสั่งให้ชื่อ+ลายเซ็นแอดมินขึ้นช่อง `ผู้เสนอราคา` บน PDF และเลือกเซลส์จาก dropdown
>   2026-09-08 — §2 ถูกเขียนใหม่ทั้งหัวข้อ + เพิ่มเฟส B2**
>
...
2. **ตัวตนพร็อกซี (proxy identity)** — แอดมินไม่ใช่ **ผู้ขาย (Odoo ช่อง H)** แต่เป็น
   **ผู้จัดทำใบ (Odoo ช่อง J `employee_quotations`)** ซึ่งเป็นบทบาทที่มีอยู่จริงในข้อมูล Odoo อยู่แล้ว
   ...
   (ชื่อ+เบอร์+ลายเซ็นของเขา) · ช่อง `ผู้เสนอราคา` = **แอดมิน + ลายเซ็นของแอดมินเอง** ·
...
| **B2** | **ชื่อ + ลายเซ็นแอดมินในช่อง `ผู้เสนอราคา` ของ PDF** (§2.5–2.7) | ❌ ไม่ — แตะ `pdfGenerator.ts` จริง แต่ทางใหม่เปิดเฉพาะใบที่มี `issuer_name` ซึ่งใบ LINE ไม่มีวันมี ⇒ ใบเก่า/ใบ LINE เหมือนเดิมทุกไบต์ (ด่าน `diag:pdf-issuer`) |
...
| **ผู้เสนอราคา** | ช่องลายเซ็นขวาของ PDF (`ผู้เสนอราคา`) | เซลส์ (คนเดียวกับซ้าย) | **แอดมินที่ล็อกอินอยู่ + ลายเซ็นของแอดมิน** |
...
| ชื่อ + ลายเซ็นช่อง `ผู้เสนอราคา` | **เซลส์คนเดิมซ้ำ** (พฤติกรรมวันนี้) | **แอดมิน + ลายเซ็นแอดมิน** |
...
### 2.3 กลไก — Proxy identity (เฟส B · ทำเสร็จแล้ว ไม่เปลี่ยน)
...
| **snapshot `employee_details`** | 2 จุดที่ประกอบ | มีแค่ `{salesperson_id, saleperson, sale_phone}` | ✅ **ต้องเติม 2 คีย์ (§2.6)** |
...
**สรุปขนาดของงานใหม่: 3 ไฟล์เก่าถูกแตะ รวมราว 40 บรรทัด**
`pdfGenerator.ts` (แยกช่องลายเซ็น) · `services/quotationService.ts` (2 คีย์ × 2 จุด) ·
`index.ts` (route อัปโหลดลายเซ็นแอดมิน) — **`odooSaleOrderExport.ts` และหน้าประวัติยังไม่ถูกแตะ**
...
**คีย์ที่เติม (2 ตัว ทั้งคู่เป็น string ล้วน):**
...
ทั้งสองคีย์ **มีเฉพาะเมื่อ `user_id LIKE 'web:%'`** — ทางที่สั้นและกันพลาดที่สุดคือฟังก์ชันเดียว
...
> **ทำไมต้อง snapshot ทั้งชื่อ *และ* กุญแจลายเซ็น ไม่ใช่คำนวณสดจาก `user_id`**
...
| --- | --- | --- |
| รูปลายเซ็น | `sale_sigs/<รหัสเซลส์>` | `admin_sigs/<issuer_sig_key>` |
| ชื่อ | ชื่อเซลส์ + ` (PM)`/` (THT)` | **ชื่อแอดมิน + ` (PM)`/` (THT)`** |
| เบอร์โทร | เบอร์เซลส์ | **ไม่พิมพ์บรรทัดนี้เลย** |
| ป้าย `( ผู้เสนอราคา )` + ช่องวันที่ | เหมือนเดิม | เหมือนเดิม |

**สามเรื่องที่ตัดสินไว้ตรงนี้ (ทั้งสามพลิกได้ด้วยการแก้บรรทัดเดียว):**

1. **เบอร์โทรของแอดมิน — ไม่พิมพ์**
   `admin_users` ไม่มีคอลัมน์เบอร์โทร และ**ไม่มีอะไรหายไปจากกระดาษ** เพราะเบอร์ที่ลูกค้าต้องโทรหา
   (เบอร์เซลส์) ยังอยู่ครบในช่อง `พนักงานขาย` ที่อยู่ติดกัน · ของเดิมพิมพ์เบอร์เดียวกันซ้ำสองช่องอยู่แล้ว
   ⚠️ ต้อง **ไม่พิมพ์ `<div>` นั้นเลย** ไม่ใช่พิมพ์ placeholder `( เบอร์โทร )` แบบที่โค้ดเดิมทำตอนค่าว่าง
   — ใบที่ส่งลูกค้าไม่ควรมีคำว่า "( เบอร์โทร )" โผล่
   *ถ้าเจ้าของอยากให้มีเบอร์:* เพิ่มคอลัมน์ `admin_users.phone` แล้วต่อท่อเข้า `issuer_phone` อีก 1 คีย์
...
-- เฟส B2 (ใหม่) — migrations/changes/2026-09-08_01_admin_users_signature_key.sql
-- กุญแจไฟล์ลายเซ็นของแอดมิน = ชื่อไฟล์ใน data/admin_sigs/ (hex 12 ตัวสุ่ม ไม่ใช่ admin_id)
-- NULL = ยังไม่เคยอัปโหลดลายเซ็น
ALTER TABLE public.admin_users ADD COLUMN IF NOT EXISTS signature_key character varying(32);
```

ทั้งคู่ NULL = ยังตั้งค่าไม่ครบ แล้วยุบเข้า `migrations/schema.sql` ตามกติกาเดิม

> **ความพร้อมของแอดมิน 1 คน = 2 อย่าง** `employee_quotation_id` (ชื่อ) + `signature_key` (ลายเซ็น)
> **ตัดสินใจ: ขาดอย่างใดอย่างหนึ่ง = ออกใบไม่ได้** (`is_ready = false` → หน้าเว็บบล็อกตั้งแต่ปุ่มร่างใบ)
> เหตุผล: เจ้าของสั่งว่า "ใน PDF แอดมินต้องมีชื่อ**และ**ลายเซ็น" และแอดมินแก้เองได้ในหน้าเดียวกัน
> ภายใน 10 วินาที — ต่างจากลายเซ็นเซลส์ที่เจ้าตัวอัปโหลดเองไม่ได้ (แอดมินเป็นคนอัปให้)
> *ถ้าอยากผ่อนเป็นแค่คำเตือน:* แก้เงื่อนไข `is_ready` บรรทัดเดียวใน `GET /webquote/me`
...
* **เซลส์ที่เลือกยังไม่มีลายเซ็น** → ช่อง `พนักงานขาย` ไม่มีลายเซ็น = **พฤติกรรมเดิมตอนเซลส์คนนั้นออกใบเอง**
  ...
  — **เตือน ไม่บล็อก** ต่างจากลายเซ็นแอดมินที่บล็อก เพราะแอดมินแก้ของตัวเองได้ทันที แต่แก้ของเซลส์ไม่ได้ในหน้านี้
...
| **B2** | **ตัวตนผู้เสนอราคาบน PDF** — ลายเซ็นแอดมิน + snapshot 2 คีย์ + แยกช่องลายเซ็น + เปลี่ยนชื่อ route `webchat`→`webquote` (ขั้น 2b) | 1 | B | ✅ ยังไม่มี UI เรียก · PDF ของ LINE ไม่ขยับ | dev | ⬜ |
...
### ขั้น 2b — ตัวตนผู้เสนอราคาบน PDF (ชื่อ + ลายเซ็นแอดมิน)  **[เฟส B2]**

migration: `admin_users.signature_key` (§2.8) แล้วยุบเข้า `migrations/schema.sql`
...
getIssuerSnapshot(userId)          → null ถ้าไม่ใช่ 'web:%' · ไม่งั้นคืน { issuer_name, issuer_sig_key }
...
**2. `services/quotationService.ts` — เติม 2 คีย์ที่จุดประกอบ snapshot ทั้ง 2 จุด**
...
เติม 2 บรรทัดข้าง ๆ `salesperson_name` / `salesperson_phone` (บรรทัด ~1900):
`issuer_name: employeeDetails.issuer_name || null` · `issuer_sig_key: employeeDetails.issuer_sig_key || null`
...
* เติม `has_signature` / `signature_url` / `is_ready` ลงคำตอบของ `GET /api/admin/webquote/me`
...
| --- | --- | --- | --- |
| `GET /api/admin/webquote/makers` | B→**B2** | admin, subadmin | 70 ชื่อผู้จัดทำจาก Odoo |
| `GET/PUT /api/admin/webquote/me` | B→**B2** | admin, subadmin | อ่าน/ตั้ง `employee_quotation_id` — **PUT 400 ถ้าไม่อยู่ในรายชื่อ** · `GET` คืน `has_signature`/`signature_url`/`is_ready` ด้วย (B2) |
...
* **ช่อง 1 — ชื่อผู้เสนอราคา/ผู้จัดทำ** dropdown ค้นหาได้จาก `GET /webquote/makers` (70 ชื่อ)
  **ไม่มีช่องพิมพ์ชื่ออิสระ** (ฝั่ง server ตอบ 400 อยู่แล้ว) → `PUT /webquote/me`
* **ช่อง 2 — ลายเซ็นของตัวเอง** อัปโหลดรูป (png/jpg) + พรีวิวจาก `signature_url`
...
* **ขาดข้อใดข้อหนึ่ง = `is_ready:false` → บล็อกทั้งหน้า** พร้อมบอกตรง ๆ ว่าขาดอะไร (§2.8)
...
* **แถบสรุปตัวตนของใบ ค้างไว้ตลอดขณะพิมพ์** เพื่อไม่ให้ออกใบผิดชื่อโดยไม่รู้ตัว:
  `พนักงานขาย: <เซลส์ที่เลือก> · ผู้เสนอราคา/ผู้จัดทำ: <ชื่อแอดมิน>`
...
| **B2** | **`diag:pdf-issuer` (ใหม่ · 6 เคส)** · `diag:pdf-render` · `diag:pdf-cache` · `diag:odoo-export` (ต้องไม่ขยับ — พิสูจน์ว่า H/J ไม่ถูกกระทบ) · `diag:company-name` (แตะ `updateQuotationCustomerSnapshot`) · Manual ข้อ 9b–9e |
...
ทั้ง 6 เคสทำงานกับ `buildSignatureBlocksHtml()` โดยตรง (สตริง เทียบได้ตรง ๆ) ไม่ต้องเปิด Chrome
ยกเว้นเคส 5 ที่ต้องแตะ DB จริง:
...
3. มี `issuer_name` → ช่องขวาต้องมีชื่อแอดมิน + ห้อยสังกัดตามค่ายของใบ (ทดสอบทั้ง `QP-` และ `QT-`)
   และ **ต้องไม่มีสตริง `( เบอร์โทร )` โผล่** (§2.7 ข้อ 1)
4. มี `issuer_name` แต่ `issuer_sig_key` เป็น `null` (ยังไม่อัปโหลด/ไฟล์หาย) → ต้องได้ชื่อ ไม่มีรูป
   **ห้าม throw และห้ามถอยไปใช้ลายเซ็นเซลส์** — ลายเซ็นคนอื่นใต้ชื่อเรา ผิดร้ายแรงกว่าไม่มีลายเซ็น
5. **`updateQuotationCustomerSnapshot()` ต้องไม่กิน 2 คีย์ทิ้ง** — สร้างใบ `web:%` จริงในDB → เรียก
   ฟังก์ชันนั้น → อ่าน `employee_details` กลับมาต้องยังมี `issuer_name` ครบ (กับดัก §2.6)
...
2. เปิด PDF เช็ค **2 ช่องต้องเป็นคนละคน** (v5.1) — ช่อง `พนักงานขาย` = ชื่อ+เบอร์+ลายเซ็นของ
   **เซลส์ที่เลือก** · ช่อง `ผู้เสนอราคา` = ชื่อ+ลายเซ็นของ**แอดมินที่ล็อกอินอยู่** และ**ไม่มีบรรทัดเบอร์โทร**
...
9e. **แอดมินยังไม่ตั้งชื่อ/ยังไม่อัปลายเซ็น** → `GET /webquote/me` ต้องได้ `is_ready:false`
   และหน้าเว็บต้องบล็อกไม่ให้กดสร้างร่าง (ตรวจทั้ง 2 กรณีแยกกัน)
...
| **แก้ไฟล์เดิม (เฟส B2)** | `pdfGenerator.ts` (**+~30 / ย้าย ~25** — แยกช่องลายเซ็นเป็น `buildSignatureBlocksHtml()`) · `services/quotationService.ts` (+~6 — 2 คีย์ × 2 จุดประกอบ snapshot + 2 บรรทัดใน `enrichQuotationData`) · `services/webIdentity.ts` (+~90) · `index.ts` (+~90 · 2 route ลายเซ็น) |
...
| migration | **8 ไฟล์** — `admin_users.employee_quotation_id` ✅ · **`admin_users.signature_key`** · `quotation_overrides` · `shipping_fee_name_presets` · `local_contacts` + `odoo_added_at` + Arm 3 · **`quotations.customer_sales_team`** · **`quotation_issue_warnings`** · audit triggers ของตารางใหม่ (ทั้งหมด additive) |
...
* **เบอร์โทรในช่อง `ผู้เสนอราคา`: ไม่พิมพ์** (เบอร์เซลส์ยังอยู่ในช่องข้าง ๆ) · **ห้อยสังกัด `(PM)/(THT)`
  ให้ชื่อแอดมินด้วย** กติกาเดียวกับชื่อเซลส์ · **ขาดชื่อหรือลายเซ็น = ออกใบไม่ได้** (§2.7, §2.8)
...
| ต้องมีเบอร์โทรของแอดมินบน PDF ไหม (ตอนนี้ตัดสินว่า **ไม่พิมพ์** — ถ้าต้องการ ต้องเพิ่ม `admin_users.phone` + คีย์ `issuer_phone` อีก 1 ตัว) | **เฟส B2** ตอนเห็น PDF จริงใบแรก |
...
→ **เฟส B2** (ต่อจากเฟส B ที่ทำเสร็จแล้ว · migration 1 ไฟล์ · ตรวจได้ครบบนเครื่อง dev)
หรือ **เฟส C** ถ้าอยากเคลียร์ของยากก่อน (แต่ด่าน LINE ต้องปิดที่ server)
· ~~เฟส A~~ ยกเลิกไปแล้วตั้งแต่ v5
