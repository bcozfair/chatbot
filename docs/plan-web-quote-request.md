...
2. **ตัวตนพร็อกซี (proxy identity)** — แอดมินไม่ใช่ **ผู้ขาย (Odoo ช่อง H)** แต่เป็น
   **ผู้จัดทำใบ (Odoo ช่อง J `employee_quotations`)** ซึ่งเป็นบทบาทที่มีอยู่จริงในข้อมูล Odoo อยู่แล้ว
   ...
   · **ชื่อและเบอร์ของแอดมินเลือกจากข้อมูลจริงใน `sale_orders` เท่านั้น ไม่มีช่องพิมพ์อิสระ** (§2.5b)
...
### 2.5b เบอร์โทรของผู้เสนอราคา — มาจากข้อมูลจริง ไม่ใช่ให้พิมพ์เอง
...
| ชื่อที่ **ไม่มีเบอร์เลย** | **2** |

⇒ **คู่ชื่อ–เบอร์ไม่ได้เป็น 1:1** สมมติฐานว่า "รู้ชื่อแล้วรู้เบอร์" ใช้ไม่ได้ ต้องออกแบบให้รองรับ

**ตรวจต่อว่าตัดสินให้อัตโนมัติได้ไหม — ตอบว่าไม่ได้ ทั้ง 3 ทาง:**

* **ผูกกับสังกัด (PM/THT) ไม่ได้** — 11 จาก 25 ชื่อยังมีหลายเบอร์*ภายในสังกัดเดียวกัน*
  ⇒ กติกา "เบอร์ตามค่ายของใบ" (แบบเดียวกับที่ห้อย `(PM)`/`(THT)`) ใช้ไม่ได้
* **เลือกเบอร์ที่ใช้บ่อยสุดให้เลย ก็ยังเสี่ยง** — สัดส่วนของเบอร์อันดับ 1 ต่ำสุดอยู่ที่ **52.2%**
  (`สถาพร รัตนประเสริฐ`: `0-2693-7005 (Auto lines)` 12 ครั้ง vs `0-2708-2669` 11 ครั้ง)
* **เบอร์ที่ใช้บ่อยสุด ≠ เบอร์ล่าสุด** ใน 6 จาก 25 ชื่อ ⇒ เอาความใหม่มาชี้ขาดก็ไม่ได้เหมือนกัน

**แต่ความกำกวมจริงน้อยกว่า 25 ชื่อ** — คู่ที่เจอบ่อยคือเบอร์เดียวกันเขียนคนละแบบ
(`0-2708-2669` vs `02-708-2669` · `02-520-3404-6` vs `02-520-3404` · `0-2451-6338-41` vs `0-2451-6338`)
หรือ **เบอร์กลางบริษัท vs เบอร์ตรง/ต่อภายใน** (`0-2693-7005 (Auto lines)` vs `0-2693-7005 Ext.315`)
ซึ่งเป็นเรื่องที่เจ้าตัวตอบได้ทันทีว่าจะให้ลูกค้าโทรเบอร์ไหน

**คำตอบ: ให้แอดมินเลือก แต่เลือกได้เฉพาะเบอร์ที่มีจริงของชื่อตัวเอง**

```
เลือกชื่อจาก dropdown  →  ระบบดึงเบอร์ของชื่อนั้นมาแสดงทั้งหมด (เรียงตามจำนวนใบที่ใช้ มาก→น้อย)
   มี 1 เบอร์  → เลือกให้อัตโนมัติ ไม่ต้องกดอะไร (43 จาก 70 ชื่อ)
   มี >1 เบอร์ → เลือกเอง โดยตัวที่ใช้บ่อยสุดถูก preselect ไว้ให้ + โชว์จำนวนใบกำกับทุกตัวเลือก
   ไม่มีเบอร์  → ข้ามไป บันทึกเป็น NULL แล้ว PDF ไม่พิมพ์บรรทัดเบอร์ (2 จาก 70 ชื่อ)
```

* **ไม่มีช่องพิมพ์เบอร์อิสระ** — เหตุผลเดียวกับชื่อ: เบอร์ที่ไม่มีในระบบต้นทาง = เบอร์ที่ไม่มีใครยืนยันได้
  ว่าถูก และมันไปอยู่บนกระดาษที่ส่งลูกค้า
* **ตรวจซ้ำฝั่ง server ตอน `PUT /webquote/me`** — คู่ (ชื่อ, เบอร์) ต้องมีอยู่จริงในรายชื่อที่ระบบดึงมา
  ไม่งั้น `400` · **เทียบตรงตัวหลัง trim ช่องว่างเท่านั้น** ห้ามทำ normalize เบอร์ (ตัด `-`, เติม 0)
  ก่อนเทียบ ด้วยเหตุผลเดียวกับที่ §2.3 ห้ามตัดวงเล็บสังกัด — **normalize = เขียนค่าทับเงียบ ๆ**
  และรูปแบบที่พิมพ์ลงกระดาษต้องเป็นรูปแบบเดียวกับที่ Odoo เก็บไว้ ไม่ใช่รูปแบบที่เราคิดว่าสวยกว่า

**ผลกับของที่เฟส B ทำไปแล้ว** — `listOdooQuotationMakers()` ที่คืน `string[]` ต้องเปลี่ยนเป็น

```ts
interface QuotationMaker { name: string; phones: { phone: string; count: number }[] }
listOdooQuotationMakers(): Promise<QuotationMaker[]>
```

query ตัวเดิม (Parallel Seq Scan 212 ms) **เพิ่ม `GROUP BY` ชั้นเดียว ไม่ได้สแกนเพิ่ม**
และ cache TTL ตัวเดิมครอบให้อยู่แล้ว · `isValidQuotationMaker(name)` กลายเป็น
`isValidMakerPhonePair(name, phone | null)` — เบอร์ `null` ผ่านได้เฉพาะเมื่อชื่อนั้นไม่มีเบอร์เลย
...
| เดาเบอร์ให้อัตโนมัติจากชื่อ (ใช้เบอร์ที่พบบ่อยสุด) | 25 จาก 70 ชื่อมีมากกว่า 1 เบอร์ · เบอร์อันดับ 1 ของบางชื่อคุมแค่ **52%** และไม่ตรงกับเบอร์ล่าสุดใน 6 ชื่อ ⇒ เดาผิดแล้วลูกค้าโทรผิดที่ (§2.5b) |
...
listOdooQuotationMakers()          → *** แก้ของเฟส B *** เดิมคืน string[] → คืน QuotationMaker[]
                                     { name, phones: [{ phone, count }] } เรียงเบอร์ตาม count มาก→น้อย
                                     query เดิม + GROUP BY ชั้นเดียว ไม่ได้สแกนเพิ่ม · cache ตัวเดิมครอบ
isValidMakerPhonePair(name, phone) → *** แทน isValidQuotationMaker() *** เทียบตรงตัวหลัง trim
                                     phone = null ผ่านได้เฉพาะชื่อที่ไม่มีเบอร์เลย (2 จาก 70 ชื่อ)
setAdminQuotationMaker(id, n, ph)  → เขียนทั้งชื่อและเบอร์ในคำสั่งเดียว (สองค่านี้ต้องไม่หลุดจากกัน)
...
> ⚠️ **`isValidQuotationMaker()` ของเฟส B ถูกแทนที่ ไม่ใช่เพิ่มตัวใหม่ข้าง ๆ** — ปล่อยตัวเก่าไว้
> = มีทางตั้งชื่อโดยไม่ตั้งเบอร์หลงเหลืออยู่ในระบบ แล้ววันหนึ่งจะมีใบที่ชื่อกับเบอร์ไม่ใช่ของคู่กัน
...
* `GET /webquote/makers` เปลี่ยนรูปคำตอบเป็น `{ name, phones: [{phone, count}] }[]` (§2.5b)
* `PUT /webquote/me` รับ `{ employee_quotation_id, employee_quotation_phone }` — ตรวจเป็น**คู่**
...
| --- | --- | --- | --- |
| `GET /api/admin/webquote/makers` | B→**B2** | admin, subadmin | 70 ชื่อผู้จัดทำจาก Odoo **พร้อมเบอร์ของแต่ละชื่อ** `{name, phones:[{phone,count}]}` (§2.5b) |
| `GET/PUT /api/admin/webquote/me` | B→**B2** | admin, subadmin | อ่าน/ตั้ง **ชื่อ + เบอร์** (`employee_quotation_id` · `employee_quotation_phone`) — **PUT 400 ถ้าคู่ชื่อ–เบอร์ไม่มีอยู่จริง** · `GET` คืน `has_signature`/`signature_url`/`is_ready` ด้วย (B2) |
...
* **ช่อง 1b — เบอร์โทร** โผล่ทันทีที่เลือกชื่อ **ไม่ใช่ช่องพิมพ์** (§2.5b):
  * ชื่อนั้นมีเบอร์เดียว (43 จาก 70) → เติมให้เลย แสดงเป็นข้อความ ไม่ต้องกดอะไร
  * มีหลายเบอร์ (25 จาก 70) → radio/dropdown สั้น ๆ **โชว์จำนวนใบกำกับทุกตัวเลือก**
    ("`0-2693-7005 (Auto lines)` — 12 ใบ") + preselect ตัวที่ใช้บ่อยสุด
  * ไม่มีเบอร์เลย (2 จาก 70) → บอกตรง ๆ ว่า "ชื่อนี้ไม่มีเบอร์ในระบบ ใบจะไม่มีบรรทัดเบอร์"
  * ทั้งชื่อและเบอร์ส่งไปด้วยกันใน `PUT /webquote/me` ครั้งเดียว
...
| **B2** | **`diag:pdf-issuer` (ใหม่ · 6 เคส)** · `diag:pdf-render` · `diag:pdf-cache` · `diag:odoo-export` (ต้องไม่ขยับ — พิสูจน์ว่า H/J ไม่ถูกกระทบ) · `diag:company-name` (แตะ `updateQuotationCustomerSnapshot`) · Manual ข้อ 9b–9g |
...
5b. **`isValidMakerPhonePair()`** — คู่ที่มีจริง = ผ่าน · ชื่อจริง+เบอร์ของคนอื่น = ไม่ผ่าน ·
   ชื่อจริง+เบอร์ที่ normalize แล้วเท่ากันแต่เขียนคนละแบบ (`02-708-2669` เทียบ `0-2708-2669`) =
   **ต้องไม่ผ่าน** ถ้ารูปแบบนั้นไม่มีอยู่จริงของชื่อนั้น · ชื่อที่ไม่มีเบอร์ + `null` = ผ่าน
...
9g. **เลือกชื่อที่มีหลายเบอร์** (เช่น `ยุพาทิพย์  ผลรัก`) → UI ต้องให้เลือกเบอร์พร้อมจำนวนใบกำกับ ·
   ส่งคู่ชื่อ–เบอร์ที่ไม่มีจริงด้วย curl ต้องได้ `400` · เลือกชื่อที่ไม่มีเบอร์เลย → บันทึกผ่านด้วยเบอร์ `null`
...
| **แก้ไฟล์เดิม (เฟส B2)** | `pdfGenerator.ts` (**+~32 / ย้าย ~25** — แยกช่องลายเซ็นเป็น `buildSignatureBlocksHtml()`) · `services/quotationService.ts` (+~8 — 3 คีย์ × 2 จุดประกอบ snapshot + 3 บรรทัดใน `enrichQuotationData`) · `services/webIdentity.ts` (+~110 · รวมแก้ `listOdooQuotationMakers` ให้คืนเบอร์ด้วย) · `index.ts` (+~95 · 2 route ลายเซ็น + รูปคำตอบใหม่ของ `makers`/`me`) |
...
* **เบอร์โทรในช่อง `ผู้เสนอราคา`: พิมพ์ ✅ ดึงจาก `sale_orders.employee_quotations_phone`**
  เก็บคู่กับชื่อตอนบันทึกโปรไฟล์ · เลือกจากเบอร์ที่มีจริงของชื่อนั้น ไม่มีช่องพิมพ์อิสระ ไม่ normalize
  (เจ้าของเคาะ 2026-09-08 · หลักฐานว่าทำไมต้องให้เลือก ไม่ใช่เดาให้ อยู่ใน §2.5b)
...
| เบอร์ที่เลือกไว้ควรตามข้อมูล Odoo ที่ sync เข้ามาใหม่โดยอัตโนมัติไหม (ตอนนี้ตรึงไว้ตอนบันทึกโปรไฟล์ — เบอร์เปลี่ยนต้องเข้ามาแก้เอง) | **เฟส B2** ตอนใช้จริงสักพัก |
